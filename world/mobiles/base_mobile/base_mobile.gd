...
var is_player = false setget ,get_is_player
...
func on_hit(_theirbox):
	return
	
func on_parried(_theirbox):
	return
	
func on_got_parried(_theirbox):
	return
	
func on_got_hit(_theirbox):
	return
...
func get_is_player():
	return Network.get_nid() == net_stats.netID
...
func get_save_data():
	...
	return  Data.get_char_data().duplicate(true)
	
	
func init_equipped(data):
	for i in data.equipped:
		var it = Data.get_item(data.equipped[i])
		equip(it)
	
	
func init_inventory(data):
...
func setup() -> void:
	...
	if !self.is_player:
		...
		var char_data = get_save_data()
		base_defaults = char_data.defaults.duplicate(true)
		init_equipped(char_data)
		init_defaults()
		init_inventory(char_data)
		init_naked()
